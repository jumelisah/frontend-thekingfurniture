...
import { getTransactionUser } from "../redux/actions/transactionUser";
...
const MyOrder = () => {
  const {auth, transactionUser} = useSelector(state=>state)
  const dispatch = useDispatch()

  useEffect(()=>{
    if (auth.userData?.id) {
      const token = window.localStorage.getItem('token')
      dispatch(getTransactionUser(token))
    }
  },[auth.userData.id])
  ...
  return (
    <Layout>
            <Head>
            <title>The King | My History</title>
            ...
            <div className={`${styles.containers} `}>
                <div className={`${styles.profiles} `}>
                    <div className="d-flex flex-column justify-content-center vh-50">
                        <div className={`${styles.content} text-center mt-4`}>My History</div>
        ...
        {transactionUser?.data && <div>
        {transactionUser?.data.map((datas, index) => {
          return (
